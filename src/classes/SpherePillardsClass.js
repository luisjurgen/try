...
class SpherePillardsClass {
   ...
   init(scene) {
       ...
       const gTex = this.textLoader.load('./assets/textures/grayMetal.png');
       const bTex = this.textLoader.load('./assets/textures/blackMetal.png');
       ...
       sphereFolder.add(this.params,'subDiv',1,5).step(1).name('Ico Subdivions').onChange(this.computePositions);
   ...
   update() {
      if(SoundReactor.playFlag){
         ...
         while (i<this.pillards.children.length){
            this.pillards.children[i].children[0].position.y = SoundReactor.fdata[i]/255*5;
            i++;
          }  
      }else{
         let i = 0;
         while (i<this.pillards.children.length){
            // this.pillards.children[i].children[0].position.y = 0;
